// 骗子酒馆对局快照存储（独立断连接续用，同 devilRouletteResumeStore 模式）。
// 把进行中的对局序列化为 JSON 存在 data/mystery/liarsBarActiveGames.json：
//   - 每次渲染/状态变更后 save(gameId, snapshot)；
//   - 对局真正收尾后 remove(gameId)；
//   - 启动时 list() 读回全部未完成对局，恢复面板继续打。
// 写入走「临时文件 + rename」原子替换 + 串行队列，避免进程崩溃留下半截文件。
...
function logFailure(operation, error) {
    console.error(`[LiarsBarResume] ${operation} failed:`, error);
}

function createLiarsBarResumeStore({ filePath, now = Date.now } = {}) {
...
const defaultStore = createLiarsBarResumeStore({
    filePath: path.join('data', 'mystery', 'liarsBarActiveGames.json'),
});

module.exports = {
    createLiarsBarResumeStore,
