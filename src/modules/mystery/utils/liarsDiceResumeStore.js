// 骗子骰子对局快照存储（独立断连接续用，同 devilRouletteResumeStore 模式）。
// 把进行中的对局序列化为 JSON 存在 data/mystery/liarsDiceActiveGames.json：
...
function logFailure(operation, error) {
    console.error(`[LiarsDiceResume] ${operation} failed:`, error);
}

function createLiarsDiceResumeStore({ filePath, now = Date.now } = {}) {
...
const defaultStore = createLiarsDiceResumeStore({
    filePath: path.join('data', 'mystery', 'liarsDiceActiveGames.json'),
});

module.exports = {
    createLiarsDiceResumeStore,
